...
**(a) Complete path search** — `core/search.py`. Runs ONCE per compiled twin, cached.
State is `(node, frozenset(capabilities_held))`; dedupe on the full state, not the node.
Capabilities only accumulate. DFS simple paths to `agent.target`. Hard caps `max_depth=8`,
`max_paths=5000`, raise `SearchBudgetExceeded` rather than hang. Produces the **path
inventory** and, compiled with `naive=True`, the headline "N critical paths" number.

**(b) Plan-then-execute agent** — `core/walk.py`. Used for ALL Monte Carlo statistics.
...
def clone(twin, *, add_controls=(), add_edges=(), remove_edges=(), add_grants=()) -> Twin   # core/twin.py (A)
def search(edges, agent) -> Inventory                                           # core/search.py   (A)
def route_policy(inventory, agent) -> tuple[RouteChoice, ...]                   # core/walk.py     (A)
def simulate(twin, agent, n: int, seed: int) -> Result                          # core/walk.py     (A)
def diff(before: Result, after: Result) -> Delta                                # core/results.py  (A)
...
def blast_radius(twin, asset_id) -> Blast                                       # core/blast.py    (A)
```

...
│
├── scripts/
│   └── gen_types.py              C   models.py JSON schema -> frontend/src/types.ts
│
├── backend/
│   ├── core/                     ══ OWNER: A (engine) ══
...
│   │
│   ├── data/                     ══ OWNER: D ══
│   │   ├── scenarios/golden.json the FinBank demo twin
│   │   ├── scenarios/golden_sync.json   golden + contractor admin@jump-01 (sync demo)
...
- **A — engine owner.** Correctness of the two algorithms over `CompiledEdge`. Reads every
  line of `core/` personally, including AI-generated lines. **A answers the judges'
  algorithm questions.**
...
4. Everyone else stops, pulls `v2/main`, rebases, and confirms their branch still builds
   before continuing.
...
`v2/main` is always demoable. That is the whole rule; the rest is mechanics.

- Integration branch is `v2/main`, cut from the existing repo's `main`. Nothing from the
...
- Branch off `v2/main`, never off another feature branch.
...
- Rebase on `v2/main` before opening a PR: `git fetch origin && git rebase origin/v2/main`.
- Never force-push `v2/main`. Force-push your own branch freely.
...
- **These always require a real review, no timeout:** anything touching `models.py`,
  `techniques.yaml`, `core/search.py`, `core/walk.py`, or `rules/compile.py`. Wrong logic
...
- Squash-merge so `v2/main`'s history stays readable.
...
|---|---|---|
| G0 — Contract | 1.5 | `models.py` v2.1, `techniques.yaml` (10), `docs/INTERFACES.md`, `golden.json` v0 on `v2/main`; `types.ts` generated; all four have pulled and pass the three P0 commands |
| G1 — Skeleton | 4 | `/simulate` returns a hardcoded `Result`; the decision card renders it from the real API, mocks off |
| G2 — Engine | 9 | `test_fixture.py`, `test_invariants.py`, `test_rules.py` green on `v2/main` |
...
- When you merge something others depend on, post it: *"evaluate_change is on v2/main."*
...
- Nobody lets a session edit `models.py` or `techniques.yaml` without the §2 protocol.
  ...
  you will type.
- Pull `v2/main` before starting a session, so it reasons about current code rather than
...
countermeasure is the ownership rule plus A personally reading `core/` and B personally
...
- **`v2/main` is broken:** it is everyone's top priority until it is green. Nobody starts
  new work on a broken `v2/main`.
...
1. **Session opener** — paste verbatim, every session, every time:
   ...
   > `<dirs>`. `backend/core/models.py` and `backend/rules/techniques.yaml` are frozen. If
...
2. **`docs/INTERFACES.md` is the only channel between sessions.** It holds the four
   ...
   line in the team channel: `[A] route_policy(inventory, agent) is on v2/main`. Everyone
...
3. **Consumers own their stubs.** Each track keeps a stand-in for what it consumes *inside
   ...
   - C: `frontend/src/api/mocks.ts` (fake `Result`, fake `ChangeVerdict`)
   - D: `backend/api/stubs.py` (hardcoded `simulate`, `evaluate_change`)
   - B: `backend/rules/_stub_policy.py` (fixed `route_policy` table until A merges)
...
4. **Handoff message is three lines**: what merged · the signature · the command that proves
   it. Example:
   > `compile()` is on v2/main · `compile(twin, techniques, *, naive=False) -> tuple[CompiledEdge, ...]` ·
